...
部署必须保留 ready 等待、同时交付适配器，并移除旧启用 shim 的配置、完整重启。[bundle 分发](../architecture/2026-09-24-distributable-dsh-bundle.md)统一交付这两个文件并提供 ready 接线，文件部署仍使用独立示例。操作规则不强制阻断任意脚本；旧会话可保留旧版本和 Shell 状态，配置切换使用新会话验收。
