...
本仓库采用独立 Git 历史，源码迁入基线为来源项目提交 `bb56fb6`。保持[主插件](../../../../dsh-default-overrides.mjs)、[Git Bash 适配器](../../../../gitbash-executor.mjs)和[针对性验证脚本](../../../../verify-dsh-default-overrides.mjs)的名称与内容不变，运行入口和相对导入继续成立。原项目保留已有文件，后续维护以本仓库为准，不做双向同步。
...
- **同时改目录布局、打包为 npm 插件并加入 CI。** 可提前准备发布自动化，但尚无 npm 发布要求和确定的 Windows 验证环境；先保持已验证的相对导入和本地文件部署。
...
交付检查比较三个运行/验证文件与来源提交的字节内容，校验示例 YAML 与 README 一致性、Markdown 相对链接和笔记格式，确认仓库不含原工作区绝对路径及无关交付。Git 历史从本地初始提交开始，不推送远端。
